...
quit;
proc sql ;
create table cm_5 as select 在营 as 纬度,sum(贷款余额_M1)/sum(贷款余额_1月前_C)as cm1,sum(贷款余额_M1) as 贷款余额_M1,sum(贷款余额_1月前_C) as 贷款余额_1月前_C  from cs_1 group by 在营;
quit;



data cm_;
set cm_1 cm_2 cm_3 cm_4 cm_5;
drop 贷款余额_M1;
run ;
...
quit;
proc sql ;
create table cm2_5 as select 在营 as 纬度,sum(贷款余额_M2)/sum(贷款余额_2月前_C) as cm2 ,sum(贷款余额_M2) as 贷款余额_M2 from cs_1 group by 在营;
quit;

data cm2;
set cm2_1 cm2_2 cm2_3 cm2_4 cm2_5;
run ;
proc sql;
