...
*统计各营业部客服姓名;
proc sql;
create table kefuxm as
select 营业部,userName,count(userName) as liang
from cs_table1_tab1_xx
where 联系月份="201812" and userName not in ("朱琨","杜盼辉","廖翠玲","洪高悬","张政嘉","蒋文","吴振杭","邱智超","陈秀芬",'白璐',"陈侃","陈天森",'高宏','袁明明','吴夏姣','易迁英',"胡宸玮","丁洁",'黄晓妮',"龙嘉苑","刘佳","黄丽华")
group by 营业部,userName;
quit;
proc sort data=kefuxm;by 营业部 userName;run;
...
/*if username in ("朱琨","杜盼辉","廖翠玲","洪高悬","张政嘉","蒋文","吴振杭","邱智超","陈秀芬",'白璐',"陈侃","陈天森",'高宏','袁明明','吴夏姣','易迁英',"胡宸玮","丁洁",'黄晓妮',"龙嘉苑","张洁","王佳旎","闫霜","仲秋尘");*/
drop 当天流入调整;
run;
...
/*else if username in ("胡宸玮",'夏多宜','高宏','吴夏姣','易迁英',"丁洁",'袁明明',"张洁","王佳旎","闫霜","仲秋尘") then do;*/
else if username not in ("杜盼辉","洪高悬","朱琨","张政嘉","蒋文","吴振杭","邱智超","陈秀芬",'白璐',"陈侃","陈天森",'黄晓妮',"龙嘉苑","廖翠玲","黄丽华" ) then do;
...
/*x  "D:\songts\workteam\登锋\米粒报表\MTD_Collector_Performance-营业部.xlsx"; */
filename DD DDE "EXCEL|[MTD_Collector_Performance-营业部.xlsx]Sheet1!r37c4:r87c7";
data _null_;set cs_all_xx;file DD;put 累计队列 当天队列 拨打 拨通  ;run;

filename DD DDE "EXCEL|[MTD_Collector_Performance-营业部.xlsx]Sheet1!r37c21:r87c21";
data _null_;set cs_all_xx;file DD;put 失联;run;

filename DD DDE "EXCEL|[MTD_Collector_Performance-营业部.xlsx]Sheet1!r37c11:r87c12";
data _null_;set cs_all_xx;file DD;put 承诺数 承诺金额    ;run;

filename DD DDE "EXCEL|[MTD_Collector_Performance-营业部.xlsx]Sheet1!r37c14:r87c16";
data _null_;set cs_all_xx;file DD;put 承诺且还数 承诺且还金额  未承诺却还金额    ;run;

filename DD DDE "EXCEL|[MTD_Collector_Performance-营业部.xlsx]Sheet1!r37c19:r87c20";
data _null_;set cs_all_xx;file DD;put 所有回款数 所有回款金额 ;run;

proc sql;
create table cs_all_d_br as
select a.序号,a.姓名,b.dail_sum as 拨打本人,c.dail_sum as 拨打联系人,d.dail_susum as 拨通本人,e.dail_susum as 拨通联系人 from list1 as a
...
filename DD DDE "EXCEL|[MTD_Collector_Performance-营业部.xlsx]Sheet1!r97c4:r147c7";
...
select a.序号,a.姓名,b.dail_sum,c.dail_susum from list1 as a
...
filename DD DDE "EXCEL|[MTD_Collector_Performance-营业部.xlsx]Sheet1!r37c8:r87c9";
...
filename DD DDE "EXCEL|[MTD_Collector_Performance-营业部.xlsx]Sheet1!r97c8:r147c11";
data _null_;set cs_all_d_br;file DD;put 拨打本人 拨通本人 拨打联系人 拨通联系人;run;

/***还款明细;*/
/*data mx;*/
/*set mtd_pay7;*/
/*if username in ("杜盼辉","洪高悬","朱琨","张政嘉","蒋文","吴振杭","邱智超","陈秀芬",'白璐',"陈侃","陈天森",'高宏','袁明明','吴夏姣','易迁英',"胡宸玮","丁洁",'黄晓妮',"龙嘉苑","张洁","王佳旎","闫霜","仲秋尘");*/
/*keep username contract_no CLEAR_DATE  承诺且还 客户姓名 实收金额;*/
/*run;*/
/*proc sort data=mx;by username descending 承诺且还;run;*/
/*filename DD DDE "EXCEL|[MTD_Collector_Performance-营业部.xlsx]明细!r2c1:r5000c7";*/
/*data _null_;set mx;file DD;put username contract_no 客户姓名 联系日期 CLEAR_DATE 承诺还款 欠款;run;*/
