...
if region="第一类" then 维度="一类营业部汇总";
	...
	else if region="第四类" then 维度="四类营业部汇总";
	else if region="第五类" then 维度="五类营业部汇总";
汇总="汇总";
if date>=&db.;
...
select 营业部,count(apply_code) as 进件量,sum(其他拒绝) as 征信等拒绝量,sum(旧评分) as 旧评分拒绝量,sum(新模型) as 新模型,sum(汇盾) as 汇盾,sum(天启黑名单) as 天启黑名单,
...
select 维度 as 营业部,count(apply_code) as 进件量,sum(其他拒绝) as 征信等拒绝量,sum(旧评分) as 旧评分拒绝量,sum(新模型) as 新模型,sum(汇盾) as 汇盾,sum(天启黑名单) as 天启黑名单,
	sum(融360) as 融360,sum(电话邦) as 电话邦,sum(审批通过) as 审批通过量,sum(审批数量) as 审批数量,sum(自动拒绝) as 自动拒绝,sum(A通过) as A通过,
	sum(B通过) as B通过,sum(C通过) as C通过,sum(D通过) as D通过,sum(E通过) as E通过,sum(A进件) as A进件,sum(B进件) as B进件,sum(C进件) as C进件,
	sum(D进件) as D进件,sum(E进件) as E进件
...
select 汇总 as 营业部,count(apply_code) as 进件量,sum(其他拒绝) as 征信等拒绝量,sum(旧评分) as 旧评分拒绝量,sum(新模型) as 新模型,sum(汇盾) as 汇盾,sum(天启黑名单) as 天启黑名单,
	sum(融360) as 融360,sum(电话邦) as 电话邦,sum(审批通过) as 审批通过量,sum(审批数量) as 审批数量,sum(自动拒绝) as 自动拒绝,sum(A通过) as A通过,
	sum(B通过) as B通过,sum(C通过) as C通过,sum(D通过) as D通过,sum(E通过) as E通过,sum(A进件) as A进件,sum(B进件) as B进件,sum(C进件) as C进件,
	sum(D进件) as D进件,sum(E进件) as E进件
...
filename DD DDE "EXCEL|[新策略应用监控-分营业部.xlsx]营业部MTD!r27c2:r47c22";
data _null_;set test_y_4;file DD;put 进件量 征信等拒绝量 旧评分拒绝量 新模型 汇盾 天启黑名单 融360 电话邦 自动拒绝 审批数量 审批通过量 A通过 B通过 C通过 D通过 E通过 A进件 B进件 C进件 D进件 E进件;run;
